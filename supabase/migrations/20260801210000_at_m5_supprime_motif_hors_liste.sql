-- CDC V2 §17.3 — Le motif de refus « Hors liste du jour » n'existe pas.
-- Une personne absente du registre ne se voit jamais refuser l'accès : elle
-- déclenche une proposition d'inscription au portail (M23), traitée côté client.
-- On retire l'étape 6 (présence sur la liste journalière) du moteur de décision M5 :
-- l'accès en entrée est désormais gouverné par le seul badge (§17.3), sans règle de liste.
--
-- Voir 20260801090915_at_m5_poste_controle_acces.sql pour la version d'origine.
...
begin
  ...
  -- ENTRÉE : chaîne complète (§17.3). Aucune règle « liste du jour ».
