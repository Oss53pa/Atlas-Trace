-- CDC V2 M23 — inscription au portail (au poste). L'agent inscrit une personne
-- inconnue : identité + photo obligatoire. L'accès est accordé immédiatement ;
-- une ligne source=PORTAIL / statut=EN_ATTENTE est créée dans le registre du jour,
-- que le référent confirme ou conteste (M4), et que la clôture confirme par silence
-- à l'échéance (M25). Deux refus seulement : personne suspendue, photo absente.
-- Le plafond peut être dépassé : signalé, jamais bloquant.
...
begin
  ...
  -- Refus 1 : photographie absente (invariant I1).
  ...
  -- Refus 2 : personne suspendue (invariant I4) — homonyme suspendu dans l'organisation.
  if exists (
  ...
  values (v_org, v_site, v_agent, 'INSCRIPTION_PORTAIL', 'at_personnes:' || v_pers::text,
    jsonb_build_object('entreprise', v_ent, 'ligne', v_ligne, 'mouvement', v_mvt)::text, v_poste.libelle);

  return jsonb_build_object(
    'resultat', 'AUTORISE', 'personne_id', v_pers, 'ligne_id', v_ligne, 'mouvement_id', v_mvt,
    'label', v_label, 'entreprise', v_ent
  );
end; $$;
