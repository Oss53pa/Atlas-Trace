-- Véhicules autorisés — extensions : alerte d'expiration (tableau de bord) + import groupé.
-- (1) at_tableau_bord : ajoute 'vehicules_expirant' (autorisations actives expirant sous 7 j).
-- (2) at_importer_vehicules_autorises : import d'une flotte (normalisation + dédoublonnage).
-- La définition complète de at_tableau_bord est ré-appliquée ici avec le champ ajouté.

create or replace function public.at_tableau_bord(p_jours integer default 1)
...
begin
  ...
  select count(*) into v_veh_expirant
    from public.at_vehicules_autorises
   where organisation_id = any(v_orgs) and statut = 'ACTIF'
     and validite_fin is not null and validite_fin between current_date and current_date + 7;
  ...
  return jsonb_build_object(
    ...
    'vehicules_expirant', v_veh_expirant,
...
create or replace function public.at_importer_vehicules_autorises(p_entreprise_id uuid, p_lignes jsonb)
 returns jsonb language plpgsql security definer set search_path to 'public'
as $function$
declare v_org uuid; v_site uuid; v_agent text; v_imm text; v_ok int := 0; v_skip int := 0; l jsonb;
begin
  ...
  for l in select * from jsonb_array_elements(coalesce(p_lignes, '[]'::jsonb)) loop
    v_imm := public.at__norm_immat(l->>'immatriculation');
    if length(v_imm) < 3 then v_skip := v_skip + 1; continue; end if;
    if exists (select 1 from public.at_vehicules_autorises
                where organisation_id = v_org and immatriculation = v_imm and statut = 'ACTIF') then
      v_skip := v_skip + 1; continue;
    end if;
    insert into public.at_vehicules_autorises
      (organisation_id, site_id, immatriculation, entreprise_id, type, chauffeur_habituel, statut, auteur)
    values
      (v_org, v_site, v_imm, p_entreprise_id, nullif(l->>'type',''), nullif(l->>'chauffeur',''), 'ACTIF', v_agent);
    v_ok := v_ok + 1;
  end loop;

  return jsonb_build_object('ok', true, 'importes', v_ok, 'ignores', v_skip);
end;
$function$;

grant execute on function public.at_importer_vehicules_autorises(uuid,jsonb) to authenticated;
